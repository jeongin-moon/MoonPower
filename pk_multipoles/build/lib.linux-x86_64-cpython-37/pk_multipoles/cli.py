"""Command-line interface for multipole estimation."""
...
def _geometry_dict(geom) -> dict:
    if isinstance(geom, PDR1FieldConfig):
        return {
            "l_x_ran": geom.l_x_ran,
            "l_y_ran": geom.l_y_ran,
            "l_z_ran": geom.l_z_ran,
            "l_x": geom.l_x,
            "l_y": geom.l_y,
            "l_z": geom.l_z,
            "z_cut_l": geom.z_cut_l,
            "z_cut_r": geom.z_cut_r,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--randoms-path", type=Path)
    parser.add_argument("--mock-data-dir", type=Path)
    parser.add_argument("--output-dir", type=Path)
    parser.add_argument("--field", default="dex-spring")
    parser.add_argument("--bin-index", type=int, default=3, help="0-based PDR1 bin index")
    parser.add_argument(
        "--assignment",
        choices=["ngp", "cic", "tsc"],
        default="cic",
    ...
    parser.add_argument("--no-overlap-weights", action="store_true")
    parser.add_argument("--sigma", type=float, default=0.6)
    parser.add_argument("--mcut", type=float, default=12.3154)
    parser.add_argument("--n-mocks", type=int, default=50)
    parser.add_argument("--mesh-spacing", type=float, default=2.19)
    parser.add_argument("--ell-max", type=int, default=4, choices=[2, 4])
    return parser
...
def run(config: RunConfig) -> None:
    ...
    if isinstance(geom, PDR1FieldConfig):
        ...
        l_x_c, l_y_c, l_z_c = geom.l_x, geom.l_y, geom.l_z
    ...
    if config.randoms_path is None:
        parser.error("randoms_path must be set in config or via --randoms-path")

...
def get_pdr1_field_config(field: str, bin_index: int, f_max_index: int = 0) -> PDR1FieldConfig:
    """Build a PDR1FieldConfig from spring/fall presets (bin_index is 0-based)."""
    if "spring" in field:
        preset = PDR1_SPRING
    elif "fall" in field:
        preset = PDR1_FALL
...
@dataclass
class RunConfig:
    """User-facing configuration for a multipole run."""
    ...
    def resolve_geometry(self) -> Union[PDR1FieldConfig, Dict[str, Any]]:
        """Fill geometry from explicit values or PDR1 presets."""
        if all(
            v is not None
            for v in (
                self.l_x,
